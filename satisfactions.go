...
type Satisfier struct {
	...
	// transaction. Use the `CheckAfter` utility function.
	CheckAfter func(locktime uint32) (bool, error)

	Sign func(pubKey []byte) (signature []byte, available bool)
...
// Based on `InputStack` of the Bitcoin Core implementation at
// https://github.com/bitcoin/bitcoin/blob/a13f3746dccd9c4ec16d6bfe9b33ebd26e3238e1/src/script/miniscript.cpp
type satisfaction struct {
	// A list of data elements that will be pushed onto the witness stack
	witness wire.TxWitness
	// If false, there is no valid satisfaction (i.e. private key or hash preimage not available,
	// timelock not yet valid, generally not satisfiable, etc.).
	available bool
	// If true, the satisfaction is malleable by a third party.
	malleable bool
	// This satisfaction requires a signature, which means a third party cannot malleate this
	// satisfaction even if `malleable` is true. If `malleable` and `hasSig` is true, only we (the
...
func verifyLockTime(txLockTime uint32, threshold uint32, lockTime uint32) bool {
	if !((txLockTime < threshold && lockTime < threshold) ||
		...
		// Can't mix timelock types (blocks vs time).
		return false
	}
	return lockTime <= txLockTime
}

// CheckOlder checks if the OP_CHECKSEQUENCEVERIFY (BIP112, BIP68) call is satisfied given the
// locktime value.
//
// txVersion is the version of the transaction being signed. OP_CHECKSEQUENCEVERIFY requires this to
// be at least 2, otherwise the script fails.
//
// txInputSequence should be set to the sequence field of the input that is being signed. It is
// compared to the locktime value.
func CheckOlder(locktime uint32, txVersion uint32, txInputSequence uint32) bool {
	// See BIP68. Mask off non-consensus bits before doing comparisons.
	lockTimeMask := uint32(wire.SequenceLockTimeIsSeconds | wire.SequenceLockTimeMask)
	return txInputSequence&wire.SequenceLockTimeDisabled == 0 &&
		txVersion >= 2 &&
		verifyLockTime(txInputSequence&lockTimeMask,
			wire.SequenceLockTimeIsSeconds,
			locktime&lockTimeMask)
}

// CheckAfter checks if the OP_CHECKLOCKTIMEVERIFY (BIP65) call is satisfied given the locktime
// value.
//
// TxLockTime is the nLockTime of the transaction that is being signed. It is compared to the
// locktime value.
//
// txInputSequence should be set to the sequence field of the input that is being signed. According
// to BIP65, it must be smaller than 0xFFFFFFFF (maximum value) for this OP-code to not abort.
...
// based on `ProduceInput()` of the Bitcoin Core implementation at:
// https://github.com/bitcoin/bitcoin/blob/a13f3746dccd9c4ec16d6bfe9b33ebd26e3238e1/src/script/miniscript.h#L850
func satisfy(node *AST, satisfier *Satisfier) (*satisfactions, error) {
	zero := func() *satisfaction {
		...
		return &satisfaction{witness: wire.TxWitness{{}}, available: true}
	}
	one := func() *satisfaction {
		return &satisfaction{witness: wire.TxWitness{{1}}, available: true}
	}
	empty := func() *satisfaction {
		return &satisfaction{witness: wire.TxWitness{}, available: true}
	}
	unavailable := func() *satisfaction { return &satisfaction{available: false} }
	witness := func(w []byte) *satisfaction {
		return &satisfaction{witness: wire.TxWitness{w}, available: true}
	}
	...
	case f_pk_k:
		...
		key := arg.value
		if arg.value == nil {
			return nil, fmt.Errorf("empty key for %s (%s)", node.identifier, arg.identifier)
		}
	...
	case f_pk_h:
		...
		return &satisfactions{
			...
			sat:  witness(sig).setAvailable(available).and(witness(key)),
		}, nil
	case f_older:
	...
	case f_sha256, f_ripemd160, f_hash256, f_hash160:
		...
		if hashValue == nil {
			return nil, fmt.Errorf("hash value empty for %s (%s)", node.identifier, node.args[0].identifier)
		}
		preimage, available := satisfier.Preimage(node.identifier, hashValue)
		if available && len(preimage) != 32 {
			return nil, fmt.Errorf(
				"length of %s preimage of %x of expected to be 32, got %d",
	...
	case f_and_b:
		...
		return &satisfactions{
			dsat: y.dsat.and(x.dsat).or(y.sat.and(x.dsat).setMalleable(true)).or(y.dsat.and(x.sat).setMalleable(true)),
			sat:  y.sat.and(x.sat),
		}, nil
	case f_or_b:
		x, err := satisfy(node.args[0], satisfier)
		...
		return &satisfactions{
			...
			sat:  z.dsat.and(x.sat).or(z.sat.and(x.dsat)).or(z.sat.and(x.sat).setMalleable(true)),
		}, nil
	case f_or_c:
		x, err := satisfy(node.args[0], satisfier)
		if err != nil {
	...
	case f_thresh:
		...
		// TODO: make more efficient, this is a naive implementation that has 2^n loop iterations.
		for ks := 0; ks < n; ks++ {
			// Iterate over all subsets of length ks.
			for _, subset := range subsets(n, ks) {
				// The witness is the concatenation of all subexpressions, ks of which are satisfied and
				// n-ks which are dissatisfied.
				candidateSat := empty()
				for i := 0; i < n; i++ {
					if containsInt(subset, i) {
						candidateSat = subSats[i].sat.and(candidateSat)
					} else {
						candidateSat = subSats[i].dsat.and(candidateSat)
					}
				}
				if ks == int(k) {
					// If exactly k subs are satisfied, it's a valid satisfaction.
					sat = sat.or(candidateSat)
				} else {
					// Any other number of satisfied subs results in an overall dissatisfaction.
	...
	case f_multi:
		...
		// All actual signatures. If a sig is unavailable, it is left empty.
		sigs := make([][]byte, n)
		for i, arg := range node.args[1:] {
			key := arg.value
			if arg.value == nil {
		...
		// TODO: make more efficient, this is a naive implementation that has (n choose k) loop
		// iterations.
		// Iterate over all k-subsets.
		for _, subset := range subsets(n, int(k)) {
			// Candidate satisfaction for one subset of keys: `sig sig sig ...`.
			candidateSat := empty()
			for _, i := range subset {
				sigsetAvailable := len(sigs[i]) > 0
				candidateSat = candidateSat.and(witness(sigs[i]).withSig().setAvailable(sigsetAvailable))
			}
	...
	case f_wrap_j:
		...
		return &satisfactions{
			dsat: zero().setMalleable(x.dsat.available && !x.dsat.hasSig),
			sat:  x.sat,
		}, nil
	default:
