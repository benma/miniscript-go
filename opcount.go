...
func computeOpCount(node *AST) (*AST, error) {
	...
	case f_sha256, f_hash256, f_ripemd160, f_hash160:
		// TODO why invalid?
		node.opCount = ops{4, invalid, zero}
	case f_andor:
		...
		node.opCount = ops{
			...
			y.opCount.sat.and(x.opCount.sat).or(z.opCount.sat.and(x.opCount.dsat)),
		}
	case f_and_v:
	...
	case f_or_b:
		...
		node.opCount = ops{
			...
			z.opCount.dsat.and(x.opCount.dsat),
			z.opCount.dsat.and(x.opCount.sat).or(z.opCount.sat.and(x.opCount.dsat)),
		}
	case f_or_c:
		x, z := node.args[0], node.args[1]
		node.opCount = ops{
		...
		for _, subset := range subsets(n, int(k)) {
			...
			for i, arg := range node.args[1:] {
				if containsInt(subset, i) {
					candidateOps = arg.opCount.sat.and(candidateOps)
				} else {
					candidateOps = arg.opCount.dsat.and(candidateOps)
				}
			}
	...
	case f_multi:
		n := len(node.args) - 1
		node.opCount = ops{1, maxInt{valid: true, value: n}, maxInt{valid: true, value: n}}
	case f_wrap_a:
		x := node.args[0]
		node.opCount = ops{2 + x.opCount.count, x.opCount.dsat, x.opCount.sat}
	case f_wrap_s, f_wrap_c, f_wrap_n:
		x := node.args[0]
		node.opCount = ops{1 + x.opCount.count, x.opCount.dsat, x.opCount.sat}
	case f_wrap_d:
		x := node.args[0]
		node.opCount = ops{3 + x.opCount.count, zero, x.opCount.sat}
	case f_wrap_v:
		x := node.args[0]
		opVerify := 0
		...
		node.opCount = ops{3 + opVerify, invalid, x.opCount.sat}
	case f_wrap_j:
		x := node.args[0]
