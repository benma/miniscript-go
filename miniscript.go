...
func (a *AST) isValid() error {
	if a.scriptLen > maxStandardP2WSHScriptSize {
		return fmt.Errorf(
			"The script size is %v, which is larger than the maximum standard P2WSH script size of %v",
			a.scriptLen,
...
// validSatisfactions checks whether successful non-malleable satisfactions are guaranteed to be
// valid.
...
// isSaneSubexpression checks whether the apparent policy of this node matches its script
// semantics. Doesn't guarantee it is a safe script on its own.
...
// ApplyVars replaces key and hash values in the miniscript. It must be called before running
// Script() or Satisfy().
//
...
// maxOpCount returns the maximum number of ops needed to satisfy this script non-malleably.
...
func typeCheck(node *AST) (*AST, error) {
	...
	case f_andor:
		...
		if !_x.props.d || !_x.props.u {
			return nil, fmt.Errorf(
				"wrong properties on `%s` in the first argument of `%s`", _x.identifier, node.identifier)
		}
		if _y.basicType != typeB && _y.basicType != typeK && _y.basicType != typeV {
			return nil, fmt.Errorf(
				"in `%s`, the second argument type is not B, K or V, but: %s",
				node.identifier, _y.basicType)
		}
		if _z.basicType != _y.basicType {
			return nil, fmt.Errorf(
				"In `%s`, the third of the argument is not the same as the type of the second argument, which is: %s",
				node.identifier, _y.basicType)
		}
		node.basicType = _y.basicType
		node.props.z = _x.props.z && _y.props.z && _z.props.z
		node.props.o = (_x.props.z && _y.props.o && _z.props.o) || (_x.props.o && _y.props.z && _z.props.z)
	...
	case f_and_v:
		...
		node.props.o = (_x.props.z && _y.props.o) || (_y.props.z && _x.props.o)
	...
	case f_or_b:
		...
		if !_z.props.d {
			return nil, fmt.Errorf(
				"wrong properties on `%s`, the second argument of `%s`", _z.identifier, node.identifier)
		}
		node.basicType = typeB
		node.props.z = _x.props.z && _z.props.z
		node.props.o = (_x.props.z && _z.props.o) || (_z.props.z && _x.props.o)
		node.props.d = true
		node.props.u = true
	case f_or_c:
		_x, _z := node.args[0], node.args[1]
	...
	case f_or_i:
		_x, _z := node.args[0], node.args[1]
		if _x.basicType != typeB && _x.basicType != typeK && _x.basicType != typeV {
			return nil, errors.New("or_i: wrong type of first argument")
		}
		if _z.basicType != _x.basicType {
			return nil, errors.New("or_i: wrong type of second argument")
		}
	...
	case f_thresh:
		...
		if !node.args[1].props.d || !node.args[1].props.u {
			return nil, fmt.Errorf(
				"wrong properties on `%s`, the second argument of `%s`", node.args[1].identifier, node.identifier)
		}
		for i := 2; i < len(node.args); i++ {
			...
			if !arg.props.d || !arg.props.u {
				return nil, fmt.Errorf(
					"wrong properties on `%s`, argument #%d of `%s`", arg.identifier, i+1, node.identifier)
			}
		}
		...
		for _, arg := range node.args[1:] {
			...
			node.props.o = node.props.o && arg.props.z && !(arg.props.o || arg.props.d || arg.props.u)
		}
		node.props.d = true
		node.props.u = true
	case f_multi:
	...
	case f_wrap_d:
		...
		if !_x.props.z {
			return nil, fmt.Errorf(
				"wrong property of `%s`, the first argument of `%s`", _x.identifier, node.identifier)
		}
		node.basicType = typeB
		node.props.o = true
		node.props.n = true
	...
	case f_wrap_n:
		...
		node.props.u = true
	default:
		return nil, fmt.Errorf("unknown identifier: %s", node.identifier)
	}
	return node, nil
}
...
func malleabilityCheck(node *AST) (*AST, error) {
	...
	case f_sha256, f_ripemd160, f_hash256, f_hash160:
		node.props.m = true
	case f_andor:
		_x, _y, _z := node.args[0].props, node.args[1].props, node.args[2].props
		node.props.m = _x.m && _y.m && _z.m && (_x.e && (_x.s || _y.s || _z.s))
	...
	case f_or_d:
		...
		// In case of `m==false` (all satisfactions are malleable), `e` can have a different
		// type in each of the two possible impementations, but it does not matter, as its
		// purpose is to compute `m`.
...
// Parse a miniscript expression, assuming it will be executed in P2WSH. The resulting node is
// checked to be a valid base expression (type "B") and that the script size does not exceed 3600
// bytes, which is a standardness rule.
...
// two opcodes `OP_CHECKSIG VERIFY` can be collapsed into one opcode `OP_CHECKSIGVERICY` (same for
...
func buildScript(node *AST, b *txscript.ScriptBuilder, collapseVerify bool) error {
	...
	case f_sha256, f_hash256, f_ripemd160, f_hash160:
		...
		if hashValue == nil {
			return fmt.Errorf("hash value empty for %s (%s)", node.identifier, node.args[0].identifier)
		}
	...
	case f_andor:
		...
		if err := buildScript(node.args[2], b, collapseVerify); err != nil {
			return err
		}
	...
	case f_thresh:
		...
		for i := 1; i < len(node.args); i++ {
			if err := buildScript(node.args[i], b, collapseVerify); err != nil {
				return err
			}
			if i > 1 {
...
// Outputs a human readable version of the script for debugging purposes.
// collapseVerify is true if the `v` wrapper (VERIFY wrapper) is an ancestor of the node. If so, the
...
func scriptStr(node *AST, collapseVerify bool) string {
	...
	case f_multi:
		...
		return strings.Join(s, " ")
	case f_wrap_a:
		return fmt.Sprintf("TOALTSTACK %s FROMALTSTACK", scriptStr(node.args[0], collapseVerify))
	case f_wrap_s:
		return fmt.Sprintf("SWAP %s", scriptStr(node.args[0], collapseVerify))
	case f_wrap_c:
		opVerify := "CHECKSIG"
		if node.props.canCollapseVerify && collapseVerify {
...
// Satisfy returns a valid non-malleable witness for this miniscript, given the available secrets
// (private keys and hash preimages). If no such witness could be found, an error is returned.
//
// The witness returned is a list of witness elements, each of which should be pushed onto the
// witness stack as a data push.
